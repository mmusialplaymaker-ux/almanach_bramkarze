...
@st.cache_data(show_spinner=False)
def _total_metrics_placeholder():
    pass
...
def build(_stats, _matches):
    ...
    if _secret("PM_BEZ_DZIEWCZYNEK", "") in ("1", "true", "True"):
        df = df[~df["zawodnik"].map(_is_girl)].reset_index(drop=True)
    _rocz = _secret("PM_ROCZNIKI", "")
    if _rocz:
        keep = {int(x) for x in re.findall(r"\d{4}", _rocz)}
        yr = pd.to_numeric(df.get("est_birth_year"), errors="coerce")
        df = df[yr.isin(keep)].reset_index(drop=True)
